...
L_calendar.prototype={
	...
	CreateHTML:function(){
		...
		for(var i=0;i<this.L_WDay.length;i++){
			htmlstr+="<li id=\"L_calendar-day_"+i+"\" style=\"background:#E4EAED\" onmouseover=\"this.style.background='#9DBFE1'\"  onmouseout=\"this.style.background='#E4EAED'\"></li>\r\n";
			}
		htmlstr+="</ul>\r\n";
		...
		stylestr+="#L_calendar{border:1px solid #ccc;width:158px;padding:1px;height:180px;z-index:9998;text-align:center;overflow:hidden}\r\n";
		...
		stylestr+="#L_calendar-day li{cursor:pointer;width:20px;height:20px;line-height:20px;float:left;;margin:1px;padding:0px;}\r\n";
		stylestr+="#L_calendar-control{height:25px;z-index:9998;}\r\n";
		stylestr+="#L_calendar-today{color:#fff;cursor:pointer;float:left;width:63px;height:20px;line-height:20px;margin:1px;text-align:center;background:#285E93}"
		stylestr+="</style>";
	...
	SetDay:function (yy,mm)   //主要的写程序**********
	{
  		...
  		for (i = 0; i < 39; i++)
		{
			...
			{ 
				if(i<firstday){
					da.innerHTML="<b style=\"color:#999;font-weight:normal\">" + this.L_WDay[i] + "</b>";
					month=(mm==1?12:mm-1);
					day=this.L_WDay[i];
				}
					...
					da.innerHTML="<b style=\"color:#333;font-weight:normal\">" + this.L_WDay[i] + "</b>";
					month=(mm==1?12:mm);
					day=this.L_WDay[i];
					if(document.all){
				...
				da.style.background=(yy == new Date().getFullYear()&&month==new Date().getMonth()+1&&day==new Date().getDate())? "#E4EAED":"#E4EAED";
				if(this.InputDate!=null){
					if(yy==this.InputDate.getFullYear() && month== this.InputDate.getMonth() + 1 && day==this.InputDate.getDate()){
						da.style.background="#FFCC00";
						}
					}
      		}
  		}
	...
	SetDate:function(){
		...
		if(document.all){
			...
			this.GetDateLayer().document.getElementById("L_calendar").style.overflow="hidden"
			}
		else{
			...
			DateLayer.style.overflow="hidden"
			}
