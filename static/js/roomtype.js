...
ratePlanApp.controller('ratePlanCtrl',
	...
	$scope.addChangeP=function addChangeP(d,c){
		...
		$("#"+d).mouseout(function(){
			...
			$("#"+d).next("div").delay(1000).hide(0, function() { $("#"+d).next("div").remove();});
	...
	function monthCheck(){
		...
		$scope.months[0]={"month":month,"year":year};
		for(var i=1;i<=2;i++){
			month++;
			if(month>12){
				month=1;
	...
	function dateCheck(monthvalue){
		...
		for(var i=0;i<($scope.roomrates.length);i++){
			...
			for (var j=0;j<dayprice.length;j++){
				var classstyle;var temp;var tempprice;
				if(dayprice[j]=="-1"){
					tempprice="--";
					classstyle="stop";
				}
				...
				temp={"classstyle":classstyle,"dayprice":tempprice};
				temptwo[j]=temp;		
			}
		...
		for(var i=1;i<=daynum;i++){
			...
			if(tempday==0 || tempday==6){
				temp={"day":i,"weekday":week,"textcolor":{color:'#23A7F5'}};
			}else{				
				temp={"day":i,"weekday":week,"textcolor":{color:'black'}};
			}
