...
class RatePlanModel(Base):
    ...
    def new_rate_plan(task_self, merchant_id, hotel_id, roomtype_id, name, meal_type, punish_type):
        ...
        task_self.session.add(rateplan)
        task_self.session.commit()

        roomrate = RoomRateModel.new_roomrate(hotel_id, roomtype_id, rateplan.id)
        roomrate.id
    ...
    def get_by_room(task_self, merchant_id, hotel_id, roomtype_id):
        from tasks.models.room_rate import RoomRateModel
        rateplans = task_self.session.query(RatePlanModel)\
        ...
        if rateplans:
            ...
            for rateplan in rateplans:
                roomrate = task_self.session.query(RoomRateModel)\
